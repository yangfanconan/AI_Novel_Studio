...
export const SmartDebugPanel: React.FC = () => {
  ...
  const runTests = async () => {
    ...
    const allTests = [
      ...
      },
      {
        ...
        tests: [{ name: "获取世界观列表", fn: testGetWorldViews }],
      },
      {
        category: "AI功能",
    ...
    addLog(
      "info",
      "test",
      `测试结果: ${passedCount} 通过, ${failedCount} 失败, ${flattenedTests.length - passedCount - failedCount} 跳过`
    );
  };
  ...
  const testCreateProject = async (testProjectId: string) => {
    ...
    return {
      message: `创建项目成功: ${newProject ? "ID: " + (newProject as any).id : "未知错误"}`,
    };
  };
  ...
  const testCreateChapter = async (testProjectId: string) => {
    ...
    return {
      message: `创建章节成功: ${newChapter ? "ID: " + (newChapter as any).id : "未知错误"}`,
    };
  };
  ...
  const testCreateCharacter = async (testProjectId: string) => {
    ...
    return {
      message: `创建角色成功: ${newCharacter ? "ID: " + (newCharacter as any).id : "未知错误"}`,
    };
  };
  ...
  const testCreatePlotPoint = async (testProjectId: string) => {
    ...
    return {
      message: `创建情节点成功: ${newPlotPoint ? "ID: " + (newPlotPoint as any).id : "未知错误"}`,
    };
  };
          ...
          <div className="flex-1 overflow-auto p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-md font-semibold mb-0">
                自动化测试 ({testProgress.current}/{testProgress.total})
              </h3>
              <button
                onClick={runTests}
                ...
                {filteredTests.map((test, index) => (
                  ...
                  >
                    ...
                    <div className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                      {test.message}
                      {test.duration && (
                        <span className="ml-2 text-xs text-slate-500">({test.duration}ms)</span>
                      )}
                    </div>
                  </div>
                ))}
