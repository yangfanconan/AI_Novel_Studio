...
export const WritingToolsPanel: React.FC<WritingToolsPanelProps> = ({ text }) => {
  ...
  const [activeTab, setActiveTab] = useState<'sensitive' | 'typos' | 'grammar' | 'format'>('sensitive');
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-2 py-0.5 text-xs rounded-md ${getSeverityBadge(analysis.sensitive_words.severity)}`}>
                            {analysis.sensitive_words.severity === 'high' ? '高风险' : 
                             analysis.sensitive_words.severity === 'medium' ? '中风险' : '低风险'}
                          </span>
                        </div>
                      </div>
                      {analysis.sensitive_words.total_count === 0 ? (
                        ...
                        <AlertTriangle className={`w-8 h-8 ${getSeverityColor(analysis.sensitive_words.severity)}`} />
                      )}
                    </div>
                  </div>
                      ...
                      {analysis.sensitive_words.sensitive_words.map((item, index) => (
                        <div
                          ...
                          className={`p-3 border-l-4 rounded-md ${
                            ...
                            item.severity === 'medium' ? 'bg-orange-50 border-orange-500' :
                            'bg-yellow-50 border-yellow-500'
                          }`}
                        >
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-medium">"{item.word}"</span>
                            <span className={`px-2 py-0.5 text-xs rounded-md ${getSeverityBadge(item.severity)}`}>
                ...
                <div className="space-y-3">
                  ...
                  {analysis.format.changes.length > 0 ? (
                    <div className="space-y-2">
                      ...
                      <div className="mt-4">
                        <div className="text-sm text-muted-foreground mb-2">规范化后的文本预览:</div>
