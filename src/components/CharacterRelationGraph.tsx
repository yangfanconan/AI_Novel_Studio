...
import { CharacterGraph, CharacterNode as CharNode, CharacterEdge as CharEdge, Character, GeneratedRelation } from '../types';
...
export function CharacterRelationGraph({ projectId, characters }: CharacterRelationGraphProps) {
  ...
  const updateFlowGraph = useCallback((graph: CharacterGraph, charList: Character[]) => {
  ...
  const handleConfirmAIRelations = async () => {
    ...
    try {
      ...
      const existingKeys = new Set(
        existingRelations.map(r => `${r.from}-${r.to}-${r.label}`)
      );
      ...
      for (const relation of generatedRelations) {
        ...
        if (fromId && toId) {
          ...
          if (existingKeys.has(key)) {
            console.log(`Skipping duplicate relation: ${relation.from_character_name} -> ${relation.to_character_name} (${relation.relation_type})`);
            skippedCount++;
            continue;
          }
          
          console.log(`Saving relation: ${relation.from_character_name} (${fromId}) -> ${relation.to_character_name} (${toId})`);
          ...
          console.warn(`Skipping relation: character not found - from: ${relation.from_character_name}, to: ${relation.to_character_name}`);
        }
      }
